...
export function createCSSRule(selector: string, cssText: string, style = getSharedStyleSheet()): void {
	...
	// Get the underlying HTMLStyleElement for sheet access
	const htmlElement = (style as StyleElementWrapper).getElement();
	htmlElement.sheet?.insertRule(`${selector} {${cssText}}`, 0);

	// Apply rule also to all cloned global stylesheets
	for (const clonedGlobalStylesheet of globalStylesheets.get(htmlElement) ?? []) {
...
export function removeCSSRulesContainingSelector(ruleName: string, style = getSharedStyleSheet()): void {
	...
	// Get the underlying HTMLStyleElement for sheet access
	const htmlElement = (style as StyleElementWrapper).getElement();
	const rules = getDynamicStyleSheetRules(htmlElement);
	...
	for (let i = toDelete.length - 1; i >= 0; i--) {
		htmlElement.sheet?.deleteRule(toDelete[i]);
	}
